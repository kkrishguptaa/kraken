...
export const issues = pgTable(
  ...
  (table) => [
    ...
    check('issues_status_check', sql`${table.status} IN ('draft', 'published')`),
  ],
)
...
export const subscribers = pgTable(
  ...
  (table) => [
    unique('subscribers_publication_id_email_unique').on(table.publicationId, table.email),
...
export const issueDeliveries = pgTable(
  ...
  (table) => [
    ...
    check('issue_deliveries_status_check', sql`${table.status} IN ('pending', 'sent', 'failed')`),
  ],
)
...
export const issueDeliveriesRelations = relations(issueDeliveries, ({ one }) => ({
  issue: one(issues, {
